import React, { useState } from 'react';
import { FinancialPage }  from './pages/FinancialPage.tsx';
import { CryptoPage }     from './pages/CryptoPage.tsx';
import { IoTPage }        from './pages/IoTPage.tsx';
import { EcommercePage }  from './pages/EcommercePage.tsx';
import { CanvasPage }     from './pages/CanvasPage.tsx';
import { AnalystPage }    from './pages/AnalystPage.tsx';
import { ThemeToggle }    from './components/ThemeToggle.tsx';

type Page = 'analyst' | 'financial' | 'crypto' | 'iot' | 'ecommerce' | 'canvas';

const NAV: { id: Page; label: string; icon: string }[] = [
  { id: 'analyst',    label: 'Analyst',   icon: '🧠' },
  { id: 'financial',  label: 'Stocks',    icon: '📈' },
  { id: 'crypto',     label: 'Crypto',    icon: '₿' },
  { id: 'iot',        label: 'IoT',       icon: '🌡' },
  { id: 'ecommerce',  label: 'Commerce',  icon: '🛒' },
  { id: 'canvas',     label: 'Canvas',    icon: '🎨' },
];

export default function App() {
  const [page, setPage] = useState<Page>('analyst');

  return (
    <div className="app">
      {/* Top Bar */}
      <header className="topbar">
        <div className="topbar-brand">
          <svg width="28" height="28" viewBox="0 0 32 32" fill="none">
            <rect width="32" height="32" rx="8" fill="#6366f1"/>
            <path d="M8 22 L14 10 L20 18 L26 10" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
            <circle cx="8" cy="22" r="2" fill="#10b981"/>
            <circle cx="26" cy="10" r="2" fill="#f59e0b"/>
          </svg>
          <span className="brand-name">DataFlow</span>
          <span className="brand-version">v0.3.3</span>
        </div>

        <nav className="topbar-nav">
          {NAV.map((n) => (
            <button
              key={n.id}
              className={`nav-btn ${page === n.id ? 'nav-btn--active' : ''}`}
              onClick={() => setPage(n.id)}
            >
              <span>{n.icon}</span>
              {n.label}
            </button>
          ))}
        </nav>

        <div className="topbar-links">
          <ThemeToggle />
          <a href="https://github.com/007krcs/dataflow" target="_blank" rel="noreferrer" className="gh-link">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z"/>
            </svg>
            GitHub
          </a>
        </div>
      </header>

      {/* Page Content */}
      <main className="main-content">
        {page === 'analyst'    && <AnalystPage />}
        {page === 'financial'  && <FinancialPage />}
        {page === 'crypto'     && <CryptoPage />}
        {page === 'iot'        && <IoTPage />}
        {page === 'ecommerce'  && <EcommercePage />}
        {page === 'canvas'     && <CanvasPage />}
      </main>
    </div>
  );
}
